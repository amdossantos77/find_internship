      ...
      <div className="max-w-7xl mx-auto relative z-10">
        ...
        <header className="mb-8 md:mb-12">
          <motion.div 
            ...
            className="flex flex-col lg:flex-row justify-between items-start lg:items-end gap-6 md:gap-8 mb-8 md:mb-10"
          >
            <div className="flex flex-col sm:flex-row items-start sm:items-center gap-4 md:gap-6 w-full">
              <div>
                <div className="flex items-center gap-2 mb-2 md:mb-3">
                  <div className="w-6 md:w-8 h-[2px] bg-[#00BABC]" />
                  <span className="text-[8px] md:text-[10px] font-black uppercase tracking-[0.3em] text-[#00BABC]">Explore 42 Jobs</span>
                </div>
              ...
              {user && (
                <motion.div 
                  ...
                  className="flex items-center gap-3 bg-slate-900/40 p-1.5 md:p-2 pr-4 md:pr-6 rounded-full border border-slate-800/50 backdrop-blur-sm self-start sm:self-end mb-1 md:mb-2"
                >
                  <img src={user.image} alt={user.login} className="w-8 h-8 md:w-10 md:h-10 rounded-full border-2 border-[#00BABC]/30 object-cover" />
                  <div className="flex flex-col">
                    <span className="text-[8px] md:text-[10px] font-black text-[#00BABC] uppercase tracking-wider">Auth as</span>
                    <span className="text-[10px] md:text-xs font-bold text-white lowercase">@{user.login}</span>
                  </div>
                </motion.div>
              )}
            </div>
              ...
              >
                {offers.map((offer, idx) => (
                  <motion.div 
                    ...
                    className="relative group bg-[#0f172a]/60 backdrop-blur-xl border border-slate-800/80 rounded-[2rem] md:rounded-[2.5rem] p-5 md:p-7 hover:border-[#00BABC]/30 transition-all flex flex-col shadow-2xl overflow-hidden"
                  >
                    ...
                    <a 
                      ...
                      className="absolute top-4 md:top-6 right-4 md:right-6 p-2 md:p-2.5 bg-slate-800 text-slate-400 hover:text-[#00BABC] hover:bg-[#00BABC]/10 rounded-xl md:rounded-2xl transition-all shadow-xl z-20"
                    >
                      <Eye className="w-4 h-4 md:w-5 md:h-5" />
                    </a>

                    <div className="flex flex-wrap gap-1.5 md:gap-2 mb-4 md:mb-6">
                      <span className="flex items-center gap-1 px-2.5 md:px-3 py-1 bg-[#00BABC]/10 border border-[#00BABC]/30 text-[#00BABC] text-[8px] md:text-[10px] font-black uppercase rounded-lg">
                        <Briefcase className="w-2.5 md:w-3 h-2.5 md:h-3" /> {offer.contract_type?.replace('_', ' ') || 'Internship'}
                      </span>
                      {offer.is_remote && (
                        <span className="flex items-center gap-1 px-2.5 md:px-3 py-1 bg-blue-500/10 border border-blue-500/30 text-blue-400 text-[8px] md:text-[10px] font-black uppercase rounded-lg">
                          <Globe className="w-2.5 md:w-3 h-2.5 md:h-3" /> Remote
                        </span>
                      )}
                    </div>

                    <div className="flex items-center gap-3 md:gap-4 mb-4 pr-8 md:pr-10">
                      <div className="w-10 h-10 md:w-12 md:h-12 bg-slate-800 rounded-xl md:rounded-2xl flex-shrink-0 flex items-center justify-center overflow-hidden border border-slate-700/50 group-hover:border-[#00BABC]/30 transition-all">
                        {offer.company_logo ? (
                      ...
                      <div className="flex flex-col min-w-0">
                        <span className="text-[8px] md:text-[10px] font-black text-[#00BABC] uppercase tracking-[0.2em] mb-0.5 md:mb-1 truncate">
                          {offer.company_name}
                        </span>
                        <h3 className="text-lg md:text-xl font-black leading-tight text-white truncate tracking-tight group-hover:text-[#00BABC] transition-colors">
                          {offer.title}
                        </h3>
                      </div>
                    </div>

                    <p className="text-slate-400 text-xs md:text-sm leading-relaxed mb-6 md:mb-8 flex-1 line-clamp-3 md:line-clamp-4">
                      {offer.little_description}
                    </p>

                    <div className="space-y-3 md:space-y-4 pt-5 md:pt-6 border-t border-slate-800/60 font-black uppercase tracking-widest text-slate-500 text-[8px] md:text-[10px]">
                      <div className="flex items-center gap-2 md:gap-3">
                        <div className="p-1.5 md:p-2 bg-slate-800/50 rounded-lg md:rounded-xl">
                          <MapPin className="w-3.5 md:w-4 h-3.5 md:h-4 text-[#00BABC]" />
                        </div>
                        <span className="text-[10px] md:text-xs font-bold text-slate-300 truncate tracking-tight uppercase">
                      ...
                      <div className="flex justify-between items-center">
                        <div className="flex items-center gap-1.5 md:gap-2">
                           <Calendar className="w-3.5 md:w-4 h-3.5 md:h-4" /> {new Date(offer.created_at).toLocaleDateString('pt-PT')}
                        </div>
                      </div>
                    </div>
                  </motion.div>
